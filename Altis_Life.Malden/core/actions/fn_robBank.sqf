...
/*
    File: life_fnc_robBank.sqf
    Author: its_leigham

    Description:
    Robs Bank then starts a timer to prevent it from being robbed again.
*/
params [
...
private _minCops = 5;
private _timer = 1800;
private _randomCash = 250000;
private _minCash = 500000;
...
//has the bank been robbed recently ?
if (_shop getVariable ["bankResetTimer", false]) exitWith {
        hint "You cannot rob the bank as someone has recently.";
        closeDialog 0;
};
...
_shop setVariable ["bankResetTimer", true, true];
...
_pgText ctrlSetText format["Robbing the Bank, stay within 5 meters (1%1)...","%"];
...
["The bank is being robbed!", false,"slow"] remoteExec ["life_fnc_notificationSystem", -2];
...
for "_i" from 0 to 1 step 0 do {
    ...
    _pgText ctrlSetText format["Robbing the Bank, stay within 5 meters (%1%2)...",round(_cP * 100),"%"];
...
[format["You have stolen $%1 from the Bank, Run!!",[_payout] call life_fnc_numberText],false,"slow"] call life_fnc_notificationSystem;
...
[0,format["Police News: The Bank has just been robbed, the stolen ammount of money was $%3!, if you have seen the robbers, contact police!",name _robber, _shop, [_payout] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",civilian];
...
_shop setVariable ["bankResetTimer", false, true];
