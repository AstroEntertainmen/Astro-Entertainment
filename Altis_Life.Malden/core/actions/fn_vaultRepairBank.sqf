...
/*
    File: life_fnc_vaultRepairBank.sqf
    Author: Fury

    Description:
    For resetting the bank once robbed
*/
...
while{true} do
{
    ...
    _rip = false;
    if!(alive _robber) exitWith {};
    [0,format["Police News: Repairs have begun on the Bank Vault eta 30 minutes.", call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",0];

uiSleep 1800;
_action = _shop addAction["Rob The Bank",life_fnc_robBank,"",0,false,false,"",'playerSide == civilian && vehicle player == player && player distance _target < 4'];
//_shop switchMove "";
